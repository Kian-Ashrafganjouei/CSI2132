...
app.get("/", (req, res) => {
  res.sendFile(__dirname + "/frontend_old/index.html");
});
...
app.get("/all_postals", async (req, res) => {
  try {
    ...
    const result = await client.query(
      "SELECT DISTINCT address.postalCode FROM hotel JOIN address ON hotel.addressID = address.addressID"
    );
...
app.get("/hotels", async (req, res) => {
  try {
    const client = await pool.connect();
    const result = await client.query(
      "SELECT hotel.*, address.postalCode FROM hotel JOIN address ON hotel.addressID = address.addressID"
    );
    res.json(result.rows);
    client.release();
  } catch (err) {
...
app.post("/hotels", async (req, res) => {
  ...
  try {
    ...
    const addressResult = await client.query(
      "INSERT INTO address (streetName, streetNumber, postalCode, unitNumber, cityName, countryName) VALUES ($1, $2, $3, $4, $5, $6) RETURNING addressID",
    ...
    await client.query(
      "INSERT INTO hotel (chain_name, category, number_of_rooms, addressID) VALUES ($1, $2, $3, $4)",
...
app.put("/hotels/:hotelId", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "UPDATE address SET streetName = $1, streetNumber = $2, postalCode = $3, unitNumber = $4, cityName = $5, countryName = $6 WHERE addressID = (SELECT addressID FROM hotel WHERE hotel_id = $7)",
      [
        streetName,
        streetNumber,
        postalCode,
    ...
    await client.query(
      "UPDATE hotel SET chain_name = $1, category = $2, number_of_rooms = $3 WHERE hotel_id = $4",
...
app.delete("/hotels/:hotelId", async (req, res) => {
  ...
  try {
    const client = await pool.connect();
    const result = await client.query("DELETE FROM hotel WHERE hotel_id = $1", [
      hotelId,
    ]);
...
app.get("/employees", async (req, res) => {
  try {
    const client = await pool.connect();
    const result = await client.query(
      "SELECT employee.*, address.postalCode FROM employee JOIN address ON employee.addressID = address.addressID"
    );
    res.json(result.rows);
    client.release();
  } catch (err) {
...
app.post("/employees", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "INSERT INTO employee (employeeName, role, isManager, ssnNumber, addressID) VALUES ($1, $2, $3, $4, $5)",
...
app.put("/employees/:employeeId", async (req, res) => {
  ...
  try {
    ...
    // Update data in the address table
    await client.query(
      "UPDATE address SET streetName = $1, streetNumber = $2, postalCode = $3, unitNumber = $4, cityName = $5, countryName = $6 WHERE addressID = (SELECT addressID FROM employee WHERE id = $7)",
      [
        streetName,
        streetNumber,
        postalCode,
    ...
    await client.query(
      "UPDATE employee SET employeeName = $1, role = $2, isManager = $3, ssnNumber = $4 WHERE id = $5",
...
app.get("/customers", async (req, res) => {
  try {
    const client = await pool.connect();
    const result = await client.query(
      "SELECT customer.*, address.postalCode FROM customer JOIN address ON customer.addressID = address.addressID"
    );
    res.json(result.rows);
    client.release();
  } catch (err) {
...
app.post("/customers", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "INSERT INTO customer (customerName, emailAddress, phoneNumber, cardNumber, idType, dateOfRegistration, addressID) VALUES ($1, $2, $3, $4, $5, $6, $7)",
      [
        customerName,
        emailAddress,
        phoneNumber,
...
app.put("/customers", async (req, res) => {
  ...
  try {
    ...
    // Update data in the address table
    await client.query(
      "UPDATE address SET streetName = $1, streetNumber = $2, postalCode = $3, unitNumber = $4, cityName = $5, countryName = $6 WHERE addressID = (SELECT addressID FROM customer WHERE customerName = $7 AND emailAddress = $8 AND phoneNumber = $9)",
      [
        streetName,
        streetNumber,
        postalCode,
    ...
    await client.query(
      "UPDATE customer SET customerName = $1, emailAddress = $2, phoneNumber = $3, cardNumber = $4, idType = $5, dateOfRegistration = $6 WHERE customerName = $7 AND emailAddress = $8 AND phoneNumber = $9",
      [
        customerName,
        emailAddress,
        phoneNumber,
...
app.delete("/customers", async (req, res) => {
  ...
  try {
    const client = await pool.connect();
    const result = await client.query(
      "DELETE FROM customer WHERE customerName = $1 AND emailAddress = $2 AND phoneNumber = $3",
...
app.post("/rooms", async (req, res) => {
  ...
  try {
    const client = await pool.connect();
    await client.query(
      "INSERT INTO room (roomNumber, floorNumber, hotelID, amenities, viewType, canBeExtended, stringComment, isRenting) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
      [
        roomNumber,
        floorNumber,
        hotelID,
...
app.put("/rooms/:roomNumber/:floorNumber/:hotelID", async (req, res) => {
  ...
  try {
    const client = await pool.connect();
    await client.query(
      "UPDATE room SET amenities = $3, viewType = $4, canBeExtended = $5, stringComment = $6, isRenting = $7 WHERE roomNumber = $8 AND hotelID = $2 AND floorNumber = $1",
      [
        floorNumber,
        hotelID,
        amenities,
...
app.delete("/rooms/:roomNumber/:floorNumber/:hotelID", async (req, res) => {
  ...
  try {
    const client = await pool.connect();
    await client.query(
      "DELETE FROM room WHERE roomNumber = $1 AND floorNumber = $2 AND hotelID = $3",
...
app.post("/search_rooms", async (req, res) => {
  ...
  try {
    ...
    let query =
      "SELECT room.*, hotel.category AS category, hotel.number_of_rooms AS number_of_rooms, hotel.chain_name AS chain_name, address.postalCode AS postalcode FROM room JOIN hotel ON room.hotelID = hotel.hotel_id JOIN address ON hotel.addressID = address.addressID WHERE 1 = 1"; // Start with a base query
...
app.post("/bookings", async (req, res) => {
  ...
  try {
    ...
    const addressValues = [
      ...
      req.body.address.unitNumber,
      req.body.address.cityName,
      req.body.address.countryName,
    ];
